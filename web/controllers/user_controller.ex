defmodule Echo.UserController do
  ...
  def login(conn, %{"name" => name, "password" => password, "device_token" => device_token, "device_name" => device_name}) do
    ...
    if user == nil, do: json conn, %{success: false, error: "NOT-REGISTERED", message: "A user with that name is not registered."}

    if Bcrypt.checkpw(password, user.password) do

      device = Repo.get_by(Device, %{user_id: user.id, token: device_token})
      case device do
        nil ->
